...
const (
	...
	ExecutorService_ExecuteStream_FullMethodName = "/executor.ExecutorService/ExecuteStream"
)
...
type ExecutorServiceClient interface {
	...
	ExecuteStream(ctx context.Context, in *ExecuteRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Line], error)
}
...
func (c *executorServiceClient) ExecuteStream(ctx context.Context, in *ExecuteRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Line], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ExecutorService_ServiceDesc.Streams[0], ExecutorService_ExecuteStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ExecuteRequest, Line]{ClientStream: stream}
...
type ExecutorService_ExecuteStreamClient = grpc.ServerStreamingClient[Line]
...
type ExecutorServiceServer interface {
	...
	ExecuteStream(*ExecuteRequest, grpc.ServerStreamingServer[Line]) error
...
func (UnimplementedExecutorServiceServer) ExecuteStream(*ExecuteRequest, grpc.ServerStreamingServer[Line]) error {
...
func _ExecutorService_ExecuteStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(ExecutorServiceServer).ExecuteStream(m, &grpc.GenericServerStream[ExecuteRequest, Line]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ExecutorService_ExecuteStreamServer = grpc.ServerStreamingServer[Line]
